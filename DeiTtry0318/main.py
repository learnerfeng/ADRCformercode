...
from datetime import timedelta  # 替代直接导入 datetime 模块
import os
import re
...
#data-path E:/ProjectTestDeeplearing/DeiT/datasetdeit
    ...
    #parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
     #                   help='dataset path')
   # parser.add_argument('--data-path', default='E:/ProjectTestDeeplearing/DeiT/datasetdeit', type=str,
    #                    help='dataset path')
    parser.add_argument('--data-path', default='E:/ProjectTestDeeplearing/DeiT/datasetdeit', type=str,
                        help='dataset path')
...
##增加checkpoint函数
def save_checkpoint(model, optimizer, output_dir):
    # 检查目录是否存在
    ...
    checkpoint_path = f"{output_dir}/{time_now}_deit_tiny_patch16_224_checkpoint.pth"
    output_dir = re.sub(r'[<>:"/\\|?*]', '_', output_dir)  # 去掉任何非法字符   
    try:
        # 保存模型权重
        torch.save({
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        '''调用 save_checkpoint 函数代替
        '''
        if args.output_dir:
            save_checkpoint(model_without_ddp, optimizer, args.output_dir)
    ...
    total_time_str = str(timedelta(seconds=int(total_time)))  # 确保使用正确的 timedelta
